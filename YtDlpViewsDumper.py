...
class CacheManager:
    ...
    def append(self, cache_name, records: dict | list[dict], chunk_size_override = None):
        ...
        if not self.cache_buffer[cache_name]:
    ...
    def flush_buffers(self):
        for cache_name, buffer in self.cache_buffer.items():
            ...
            for chunk_name in chunk_names:
                chunk_num_str = chunk_name[len(cache_name) + 1:len(".cache.json")]
                chunk_num = int(chunk_num_str)
                if chunk_max < chunk_num:
                    chunk_max = chunk_num
                    
            logger.debug(f"Max existing chunk number for {cache_name}: {chunk_max}")
            chunk_num_current = chunk_max + 1
            cache_chunk_name = os.path.join(cache_dir, f"{cache_name}-{chunk_num_current:6}.cache.json")
            
            with open(cache_chunk_name) as fout:
